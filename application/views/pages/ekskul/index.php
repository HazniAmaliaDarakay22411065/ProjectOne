                ...
                <div class="card border border-light shadow-lg rounded-4" style="transition: all 0.3s;">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center">
                            ...
                            <a href="<?= base_url('ekskul/create') ?>" class="btn btn-sm btn-primary ms-3">
                                <i class="fas fa-plus"></i> Tambah Ekskul
                            </a>
                        </div>

                        <!-- Form pencarian -->
                        <form action="<?= base_url("ekskul/search") ?>" method="POST" class="d-flex">
                            <div class="input-group">
                                <input type="text" name="keyword" class="form-control form-control-sm me-2" placeholder="Cari Ekskul" value="<?= $this->session->userdata('keyword') ?>">
                                <button class="btn btn-info btn-sm" type="submit">
                                    <i class="fas fa-search"></i>
                                </button>
                                <a href="<?= base_url("ekskul/reset") ?>" class="btn btn-info btn-sm ms-1">
                            ...
                            <tbody>
                                ...
                                foreach ($content as $row) : $no++; ?>
                                    <tr>
                                        ...
                                        <td><?= $row->id_ekskul ?></td>
                                        <td>
                                            <?php if (!empty($row->image) && file_exists(FCPATH . "images/ekskul/{$row->image}")) : ?>
                                                <img src="<?= base_url("images/ekskul/{$row->image}") ?>" height="50" alt="Foto Ekskul">
                                            <?php else : ?>
                                        ...
                                        <td><?= $row->guru_nama ?? '-' ?></td>
                                        <td>
                                            <a href="<?= base_url("ekskul/edit/$row->id_ekskul") ?>" class="btn btn-sm">
                                                <i class="fas fa-edit text-info"></i>
                                            </a>
                                            <?= form_open(base_url("ekskul/delete/$row->id_ekskul"), ['method' => 'POST', 'class' => 'd-inline']) ?>
