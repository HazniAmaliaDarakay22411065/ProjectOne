<section class="container py-5" style="margin-top: 6rem;"> <!-- Tambah jarak dari navbar -->
    <div class="container bg-light text-dark mb-5"
        style="max-width: 800px; margin: 0 auto; padding: 2rem; border-radius: 15px; 
               box-shadow: 0 4px 20px rgba(0, 0, 0, 0.1); 
               background: linear-gradient(to bottom right, #f8f9fa, #e9ecef);">
        <div class="container-galeri text-center">
            <h3 style="font-family: 'Rowdies', serif; font-weight: bold; color: #343a40;">BERITA TERBARU</h3>
    ...
    <div class="row">
        <?php if (!empty($berita)) : ?>
            <?php foreach ($berita as $item) : ?>
                <div class="col-sm-6 col-md-4 mb-5"> <!-- Tambah margin antar card -->
                    <div class="card shadow-sm rounded-4 overflow-hidden position-relative" style="padding-bottom: 50px;">

                        <!-- Badge Tanggal -->
                        <span class="badge bg-success position-absolute top-0 start-0 m-2 rounded-pill">
                            <?= date('d M Y', strtotime($item->tanggal ?? date('Y-m-d'))) ?>
                        </span>

                        <!-- Gambar Berita -->
                        <?php
                        $image = !empty($item->image)
                            ? base_url('images/berita/' . $item->image)
                            : base_url('images/default-berita.jpg');
                        ?>
                        <img src="<?= $image ?>" class="card-img-top" alt="Gambar Berita" style="height: 200px; object-fit: cover;">

                        <!-- Judul Berita -->
                        <div class="position-absolute start-50 translate-middle-x px-3 py-2 rounded-3 shadow-sm bg-light"
                            style="bottom: 10px; min-width: 80%; text-align: center;">
                            <h6 class="mb-0 fw-bold text-black text-decoration-underline">
                                <a href="<?= base_url("berita/detail/{$item->id_berita}") ?>" class="text-decoration-none text-black">
        ...
        <?php else : ?>
            <div class="col-12">
                <div class="alert alert-info text-center">Belum ada berita yang ditambahkan.</div>
            </div>
        <?php endif; ?>
    </div>
</section>
