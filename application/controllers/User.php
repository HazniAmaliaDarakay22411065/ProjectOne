...
class User extends MY_Controller
{
    ...
    public function index($page = null)
    {
        ...
        $data['pagination']    = $this->user->makePagination(
            base_url('user'),
            2,
            $data['total_rows'] //memberikan index buat user ttl row menentukan brp bnyk dt pagination yg d tampilkan
        );
    ...
    public function edit($id)
    {
        ...
        if (!$_POST) {
            ...
            if ($data['input']->password !== '') {
                $data['input']->password = hashEncrypt($data['input']->password); //jika dlm inputan pw ada suatu nilai mka akn d lakukan encrphy

            } else {
                $data['input']->password = $data['content']->password; //jika kosong maka makek pw yg sdh ad melalui user data pda id
            }
        }
